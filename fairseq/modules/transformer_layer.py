...
class TransformerEncoderLayerBase(nn.Module):
    ...
    *cfg.encoder.normalize_before* to ``True``.
    ...
    def forward(
        ...
        info: Optional[Tensor] = None,  # Add the info parameter
    ):
        """
        Args:
        ...
        Returns:
            Tuple[Tensor, Tensor]: encoded output of shape `(seq_len, batch, embed_dim)` and the feedforward result.
        """
        # anything in original attn_mask = 1, becomes -1e8
        # anything in original attn_mask = 0, becomes 0
        if info is not None:
            x = x + info  # Incorporate the info context
        ...
        fc_result = x  # Store the feedforward result
        ...
        if self.return_fc and not torch.jit.is_scripting():
            return x, fc_result  # Return both the encoded output and the feedforward result

        return x, fc_result  # Ensure to return both values
...
class TransformerDecoderLayerBase(nn.Module):
    ...
    *cfg.decoder.normalize_before* to ``True``.
    ...
    def apply_test_lagging(self, x):
        # Example logic for applying lagging
        # This is just a placeholder; adjust based on your specific needs
        lagging_factor = 0.5  # Example factor
        return x * lagging_factor  # Modify x based on your logic

    def forward(
        self,
        x,
        encoder_out: Optional[tuple[Tensor, ...]] = None,
        ...
        train_waitk_lagging: bool = False,
        test_waitk_lagging: bool = False,
        src_info: Optional[Tensor] = None,  # Add the src_info parameter
        tgt_info: Optional[Tensor] = None,   # Add the tgt_info parameter
    ):
        """
        Args:
            x (Tensor): input to the layer of shape `(seq_len, batch, embed_dim)`
        ...
        if train_waitk_lagging:
            x = self.apply_lagging(x)  # Implement logic for train_waitk_lagging

        if test_waitk_lagging:
            x = self.apply_test_lagging(x)  # Implement logic for test_waitk_lagging
